...
describe('HardyCLI: ', function() {
  ...
  beforeEach(function() {
    ...
    child_processMock = {
      spawn: jasmine.createSpy('child_process.spawn').andReturn({
        pid: '999',
        on: jasmine.createSpy('registering on process exit')
      })
    };
  ...
  describe('Selenium control', function() {
    ...
    it('stops selenium', function() {
      httpMock.get = function(url, success) {
        success();
        return httpMock;
      };
      httpMock.on = function(state, fail) {
      ...
      fsMock.existsSync = function(lockFile) {
        return true;
      };
